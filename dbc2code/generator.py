...
def generate_can_id_h(database, output_dir: Path):
    ...
    with open(output_dir / "can_id.h", "w", encoding="utf-8") as f:
        f.write(output_text)
...
def generate_can_signal_id_h(database, output_dir: Path):
    ...
    output_text = template.render(database=database)
    with open(output_dir / "can_signal_id.h", "w", encoding="utf-8") as f:
        f.write(output_text)
...
def generate_can_if_h(database, output_dir: Path):
    ...
    output_text = template.render(database=database)
    with open(output_dir / "can_if.h", "w", encoding="utf-8") as f:
        f.write(output_text)
...
def generate_can_if_c(database, output_dir: Path):
    ...
    output_text = template.render(database=database)
    with open(output_dir / "can_if.c", "w", encoding="utf-8") as f:
        f.write(output_text)


if __name__ == "__main__":
    with open(current_dir / "input" / "vehicle.dbc", "r", encoding="utf-8") as f:
        dbc_content = f.read()
