...
// maybe I could use malloc create the array dynamically, but for project level array is much more safer unless stack room is not enough.
// the callback may be not in the order of CAN signals, but it could contain all of the signals.
...
CanIfRet SubscribeCanSig(SubScriberInfo *info, uint32_t num) {
    if((NULL == info) || (0 >= num)) {
        // output log: please input valid subscribe config
        return CAN_IF_RET_NG;
    }
    for(int i = 0; i < num; i++) {
        callback_info[i] = info[i];
    }
}